...
/**
 * This class contains a main that runs the ASPRAlign comparison algorithm
...
public class WorkbenchComparator {
	...
	if (cmd.hasOption("f")) {
	    ...
	    String structuresStreamName = inputDirectory.getAbsolutePath()
		    + "/" + "ASPRAlignProcessedStructures.csv";
		...
		if (!structures.containsKey(f1)) {
			...
			secondaryStructure1 = RNASecondaryStructureFileReader
				.readStructure(f1.getPath(), basePairsCheck);
		    } catch (IOException e) {
		...
		while (intIt.hasNext()) {
			...
			System.err.println(
				"WARNING: Skipping the comparison of pair ("
