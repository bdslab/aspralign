...
/**
 * Builder for an algebraic RNA tree and a structural RNA tree corresponding
 * to a given RNA secondary structure.
 * 
 ...
 * structures.
 * 
 * A structural RNA tree is a tree description of the structural part of an
 * RNA secondary structure that may contain any kind of pseudoknot. It is used
 * to align secondary structures and calculate the distance between them.
 * 
 * @author Luca Tesei
 *
 */
public class ASPRATree {
    ...
    public Tree<String> getAlgebraicRNATree()
	    throws RNAInputFileParserException {
	if (this.secondaryStructure.sequence == null)
	    throw new RNAInputFileParserException(
		    "Cannot construct the algebraic RNA tree from a secondary "
	...
	if (r == this.secondaryStructure.size)
	    ...
	    tail = this.secondaryStructure.sequence.substring(r,
		    this.secondaryStructure.size);
	...
	// start the recursive construction of the algebraic RNA Tree on the
	// node ct
	...
	if (this.secondaryStructure.p[r] == l) {
	    ...
	    // decrease counting array according to the elimination of this
	    // hairpin
	    for (int i = l; i < r; i++)
		c[i]--;
	    ...
	    if (lp == rp) {
		// no subloops of this nesting, there will be no more complex
		// subtrees
		...
		String label = this.secondaryStructure.sequence.charAt(l - 1)
			...
			+ "*";
		q.setValue(label);
		// use the following line to obtain the string without the
		// "*"s
	    ...
	    // create the empty node for building the rest of the tree on the
	    // left
	    ...
	    // recursive construction of the algebraic RNA subTree on the node
	    // rest
		...
		if (rmzi.isEmpty())
		    ...
		    seq.setValue("(" + Operators.CONCATENATION_LABEL + ","
			    + this.secondaryStructure.sequence.substring(lr,
				    rl - 1)
			    + ")");
    ...
    /*
     * Initialize the counting array c. The counting array is defined as
     * follows: the count starts at zero. At any start of an hairpin loop in
     * the secondary structure the counting increases by one. At any stop of
     * an hairpin loop in the secondary structure the counting decreases by
     * one. All the positions are assigned the corresponding count. If in the
     * position there is not a start or a stop of an hairpin loop the current
     * value of count is assigned.
     */
    private void init(int[] c) {
	int count = 0;
	for (int i = 1; i <= this.secondaryStructure.size; i++) {
	    if (loop_start(i)) {
		// a loop has just started in the arc annotated sequence, so
		// count is
		// incremented
		count++;
		c[i] = count;
	    } else if (loop_stop(i)) {
		// a loop has just stopped in the arc annotated sequence, so
		// count is
		// decremented
		count--;
		c[i] = count;
	...
	return this.secondaryStructure.p[i] != 0
		&& this.secondaryStructure.p[i] > i;
    }
	...
	do {
	    ...
	    while (c[i] == 0)
		i++;
	    // determine the stop of the zero interva, if start == stop, the
	    // zero interval
	    // is empty
	    int stop = i;
	...
	/*
	 * create a copy of the list of weak bonds of the structure, ensuring
	 * that the copied list conserves ordering
	 */
    ...
    private void recBuildStructural(Tree<String> ct, List<WeakBond> bonds,
	...
	assert lastBond.getRight() == r
		: "Mismatch among indexes of secondary structure and of "
			+ "determined loop in WeakBond: (" + l + "," + r
			+ ") vs (" + lastBond.getLeft() + ","
			+ lastBond.getRight() + ")";
	...
	if (this.secondaryStructure.p[r] == l) {
		...
		// subtrees
		assert bonds.size() == 1
			: "Mismatch in base case of building structural RNA "
				+ "tree: size of list of bonds different from one";

		// revert to just a single hairpin
		ct.setValue(Operators.HAIRPIN_LABEL + "(" + lastBond.getLeft()
			+ "," + lastBond.getRight() + ")");
	    ...
	    // create hairpin subtree
	    Tree<String> h = new Tree<String>();
	    h.setValue(Operators.HAIRPIN_LABEL + "(" + lastBond.getLeft()
		    + "," + lastBond.getRight() + ")");
    ...
    private void splitBonds(List<WeakBond> bonds, int ll, int lr, int rl,
	    int rr, List<WeakBond> lbonds, List<WeakBond> rbonds) {
	for (WeakBond b : bonds) {
		...
		assert false : "Error in splitting bonds: weak bond ("
			+ b.getLeft() + "," + b.getRight()
			+ ") not in range [" + ll + "," + lr
			+ "] and not in range [" + rl + "," + rr + "]";
	}
	assert lbonds.size() > 0
		: "Error in splitting bonds: left bonds list empty";
	assert rbonds.size() > 0
		: "Error in splitting bonds: right bonds list empty";
	...
	for (int i = bonds.size() - 2; i >= 0; i--) {
	    ...
	    if (b.getLeft() < lastBondLeftIndex
		    && lastBondLeftIndex < b.getRight())
		n++;
	}
